...
class Settings(BaseSettings):
    ...
    openai_environment_fallback_enabled: bool = True
    openai_environment_search_model: str = "gpt-5.4-mini"
    openai_environment_search_timeout_seconds: float = Field(
        default=20.0, gt=1.0, le=60.0
    )
    environment_web_search_consumption_margin_percent: float = Field(
    ...
    vinfast_max_retries: int = Field(default=2, ge=0, le=5)
    vinfast_access_denied_cooldown_seconds: float = Field(default=300.0, ge=1.0)
    vinfast_rate_limit_cooldown_seconds: float = Field(default=30.0, ge=1.0)
    ...
    open_meteo_max_retries: int = Field(default=2, ge=0, le=5)
    open_meteo_retry_base_delay_seconds: float = Field(default=0.5, ge=0.0, le=10.0)
    open_meteo_cache_ttl_seconds: float = Field(default=10800.0, ge=0.0, le=86400.0)
    environment_fallback_enabled: bool = True
    environment_fallback_consumption_margin_percent: float = Field(
        default=20.0, ge=0.0, le=100.0
    )
    environment_cached_consumption_margin_percent: float = Field(
