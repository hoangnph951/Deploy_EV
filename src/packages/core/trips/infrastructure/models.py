...
class GeographyPoint(UserDefinedType):
    """PostGIS geography point with a SQLite text variant for deterministic tests."""
...
class PlanVersionModel(Base):
    ...
    assumptions: Mapped[dict] = mapped_column(JSON_DOCUMENT, nullable=False)
...
class AuditLogModel(Base):
    ...
    id: Mapped[str] = mapped_column(String(64), primary_key=True)
    trip_id: Mapped[str] = mapped_column(
        String(64), ForeignKey("trips.id", ondelete="CASCADE"), nullable=False, index=True
    )
...
class ChargingLocationModel(Base):
    ...
    __table_args__ = (
        UniqueConstraint("provider", "external_id", name="uq_charging_locations_provider_external"),
        CheckConstraint(
    ...
    location: Mapped[str] = mapped_column(
        GeographyPoint().with_variant(String(128), "sqlite"),
        nullable=False,
    )
...
class ChargingEvseModel(Base):
    ...
    external_evse_id: Mapped[str | None] = mapped_column(String(255), nullable=True)
    depot_status: Mapped[str | None] = mapped_column(String(64), nullable=True)
...
class StationExternalEvidenceModel(Base):
    ...
    raw_evidence: Mapped[dict | None] = mapped_column(JSON_DOCUMENT, nullable=True)
...
class StationGraphVersionModel(Base):
    ...
    last_location_id: Mapped[int | None] = mapped_column(BIGINT_PK, nullable=True)
    metadata_json: Mapped[dict | None] = mapped_column(JSON_DOCUMENT, nullable=True)
...
class StationEdgeModel(Base):
    ...
    geometry_polyline: Mapped[str | None] = mapped_column(Text, nullable=True)
    provider_source_url: Mapped[str | None] = mapped_column(Text, nullable=True)
...
class PlanningRunModel(Base):
    ...
    request_snapshot: Mapped[dict] = mapped_column(JSON_DOCUMENT, nullable=False)
    result_code: Mapped[str | None] = mapped_column(String(64), nullable=True)
    error_code: Mapped[str | None] = mapped_column(String(128), nullable=True)
    error_detail: Mapped[dict | None] = mapped_column(JSON_DOCUMENT, nullable=True)
